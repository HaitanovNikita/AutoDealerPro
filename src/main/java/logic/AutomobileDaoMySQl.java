...
import static logic.Utilites.colorPrintln;
...
public class AutomobileDaoMySQl implements AutomobileDao {
    ...
    public ArrayList<Automobile> getMostPopularAuto() {
        ...
        try {
            ...
            session.getTransaction().begin();
            ArrayList<Object[]> datasList = (ArrayList<Object[]>) session.createQuery(Utilites.queryForMostPopularCar + id).getResultList();
    ...
    private int auxiliaryMethod1(int id) {
        int result = 0;
        try {
            String query = Utilites.auxiliaryQuery1 + id;
            session = HibernateUtils.getSessionFactory().getCurrentSession();
            session.getTransaction().begin();
    ...
    private int auxiliaryMethod2() {
        ...
        try {
            session = HibernateUtils.getSessionFactory().getCurrentSession();
            session.getTransaction().begin();
            Object[] array = session.createQuery(Utilites.auxiliaryQuery2).getResultList().toArray();
            result = Integer.parseInt(array[0].toString());
            session.getTransaction().commit();
        } catch (Exception e) {
            e.printStackTrace();
